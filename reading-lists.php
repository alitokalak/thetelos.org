...
function tls_ajax_get_my_lists() {
    ...
    foreach ($saved as $lid) {
        ...
        // Kitap detayları
        $book_details = [];
        foreach ($books as $pid) {
            $bp = get_post($pid);
            if (!$bp) continue;
            $authors = get_the_terms($pid, 'authors');
            $author  = (!empty($authors) && !is_wp_error($authors)) ? $authors[0]->name : '';
            $cats    = get_the_category($pid);
            $color   = (function_exists('thetelos_get_category_palette') && !empty($cats))
            ...
            $book_details[] = [
                ...
                'cover'  => has_post_thumbnail($pid) ? get_the_post_thumbnail_url($pid, 'thumbnail') : '',
                'color'  => $color,
            ];
        }

...
(function(){
    ...
    /* ── Kütüphane yükle ── */
    function miniCover(title, bg) {
        var t = title.replace(/\s*[\(\[（].*?[\)\]）]/g,'').replace(/\s*[-–—].*$/,'').trim();
        if (t.length>16) t=t.substring(0,14)+'…';
        return '<div style="width:44px;height:62px;background:'+bg+';border-radius:4px;display:flex;flex-direction:column;align-items:center;justify-content:space-between;padding:5px 3px 4px;box-sizing:border-box;position:relative;overflow:hidden;flex-shrink:0;">'
            +'<div style="position:absolute;inset:3px;border:1px solid rgba(255,255,255,0.2);border-radius:2px;pointer-events:none;"></div>'
            +'<div style="color:rgba(255,255,255,0.92);font-size:6px;text-align:center;line-height:1.3;font-family:Georgia,serif;font-style:italic;z-index:1;word-break:break-word;padding:0 2px;">'+t+'</div>'
            +'<div style="color:rgba(255,255,255,0.4);font-size:4.5px;letter-spacing:.1em;font-family:Georgia,serif;z-index:1;">thetelos</div>'
            +'</div>';
    }
    ...
    function loadLib(filter) {
        ...
        .then(function(res){
            ...
            grid.innerHTML=res.data.books.map(function(b){
                var coverHtml;
                if(b.cover){
                    coverHtml='<img src="'+b.cover+'" alt="'+b.title+'" loading="lazy" onerror="this.outerHTML=miniCover(\''+b.title.replace(/'/g,"\\'").replace(/"/g,'&quot;')+'\',\''+(b.color||'#1C2B3A')+'\')">';
                } else {
                    coverHtml=miniCover(b.title,b.color||'#1C2B3A');
                }
                ...
                var booksHtml = rl.book_details && rl.book_details.length
                    ? rl.book_details.map(function(b,i){
                        var cover = b.cover
                            ...
                            : '<div class="tls-rl-prof-book-ph" style="background:'+(b.color||'#1C2B3A')+'">'+b.title.charAt(0)+'</div>';

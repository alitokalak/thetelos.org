...
async function drawCard(canvas, item, style){
  ...
  // ── ÜST MARKA (site logosu, temaya göre renklendirilmiş) ──
  const logo=await logoImg(cGold);
  const lw=150, lh0=lw*25/40, lx=(W-lw)/2, ly=58;
  if(logo){ x.drawImage(logo, lx, ly, lw, lh0); }
  else { x.fillStyle=cGold; x.font='600 24px '+SANS; x.letterSpacing='6px'; x.fillText('THETELOS', W/2, 120); x.letterSpacing='0px'; }
  const logoBottom = ly + lh0;         // ≈ 152

  // ── KAPAK GÖRSELİ (~ekranın yarısı, orantılı sığdır) ──
  let quoteTop=logoBottom+26;          // kapak yoksa alıntı bloğu logonun altından başlar
  if(cover){
    ...
    const cx=(W-cw)/2, cy=logoBottom+30;
  ...
  const yHandle=H-74, ySite=H-108, yBook=H-152, yAuthor=item.book?H-200:H-160;
  const attrTop=attr?yAuthor:(item.book?yBook:H-140);   // atıf bloğunun en üstü
  ...
  if(attr){ x.fillStyle=cGold; x.font='600 28px '+SANS; x.letterSpacing='3px'; x.fillText(attr,W/2,yAuthor); x.letterSpacing='0px'; }
  ...
  // ── ALT: site + handle (sabit konum) ──
  x.fillStyle=cMuted; x.font='500 22px '+SANS; x.letterSpacing='1px';
  x.fillText(item.site||'thetelos.org', W/2, ySite);
  x.fillStyle=cGold; x.font='600 22px '+SANS;
  x.fillText(item.handle||'@thetelos', W/2, yHandle); x.letterSpacing='0px';
}
